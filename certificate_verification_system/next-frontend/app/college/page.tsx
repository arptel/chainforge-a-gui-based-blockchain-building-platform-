...
import { issueCertificate, revokeCertificate } from '@/lib/api';
...
export default function CollegeDashboard() {
    ...
    const [revokeStatus, setRevokeStatus] = useState<any>(null);
                    ...
                    <form onSubmit={handleIssue} className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    required type="number" value={formData.year} onChange={(e) => setFormData({ ...formData, year: Number(e.target.value) })}
                                    className="w-full bg-black/50 border border-white/10 rounded-lg py-2 px-3 focus:outline-none focus:border-indigo-500 transition-colors text-white"
                                />
                            </div>
                        </div>
                    ...
                    <div className="space-y-3">
                        {recentCerts.map((c, i) => (
                            <div key={i} className="flex justify-between items-center p-3 rounded-lg bg-black/40 border border-white/5">
                                ...
                                <button onClick={() => router.push(`/student/${c.certId}`)} className="text-indigo-400 text-sm hover:text-indigo-300 underline">
                                    View Page
                                </button>
                            </div>
                        ))}
